...
@section('content')
    <div class="card shadow h-100">
        ...
        <div class="card-header p-2">
            <h4 class="h3 mb-0 text-gray-800">Quản lý loại bài đăng</h4>
        </div>
                                ...
                                <tbody>
                                    @foreach ($allTypePost as $key => $value)
                                        <tr id="tr_{{ $value->id }}">
                                            ...
                                            <td data-label="Tên loại">{{ $value->post_type_name }}</td>
                                            <td data-label="Tùy chọn">
                                                <a name="" id="" class="btn btn-primary" href="{{ route('edit_type_post_new', $value->id) }}" role="button"
                                                    title="Chỉnh sửa">
                            ...
                            <div class="card-body">
                                <form class="needs-validation" action="{{ route('add_type_post_new') }}" method="POST" novalidate>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="typeNamePost" id="typeNamePost" aria-describedby="helpId" placeholder="Nhập tên loại bài đăng" required>
                                        <small class="invalid-feedback">Vui lòng nhập tên loại bài đăng</small>
                                    </div>
    ...
    @if (Session::has('edit_type_post'))
        <script>
            Swal.fire({
                position: 'top-end',
                icon: 'success',
                title: 'Đã chỉnh sửa loại bài đăng thành công',
                showConfirmButton: false,
                timer: 2000
            });
        </script>
            ...
            $('.delete_all').on('click', function(e) {
                ...
                {  
                    ...
                    if(check == true){  
                        ...
                        $.ajax({
                            url: "{{ route('delete_all_type_post_new') }}",
                            type: 'DELETE',
